...
## 21. 阶段 2 任务 2.1 完成：Qwen3-Embedding 全面超过 e5（2026-09-18 凌晨）

### 21.1 重嵌入执行记录

```
模型      Qwen3-Embedding-0.6B（1024 维 / bf16 / max_length 2048 / last-token 池化）
语料      144,732 chunks
硬件      RTX 4060 Laptop 8 GB，21.5 chunks/s
耗时      约 1.9 小时（预期 4~6 h；原因是真实 chunk 平均远短于合成探测用的 874 token）
产物      vec/vectors.bin 565.4 MB（144,732 x 1024 x 4 + 24B 头）
          vec/vectors.meta.jsonl 40.6 MB
```

### 21.2 换模型本身不够：必须同时做"标识符定向提权"

第一次跑（融合 0.3/0.7，sk=5，无提权）：**R@10 0.4750，MRR 0.3278 —— 全面低于 e5**。
但逐条拆解显示模型其实更强：

- **候选生成缺口从 12 降到 5**（cl01–cl04 基类提问、bl02/bl04 mod Def 从"两腿都没捞到"变成命中）
- **裸标识符查询从 R@10 1.00 掉到 0.00**（`Need_Food` / `CompPowerTrader` / `JobDriver_Mine` / `ThingDef`）

也就是说：**新模型更会"找得到"，但把精确标识符排得更差**。
试过用全局词法权重补救 —— **更糟**（0.7/0.3 时 R@10 崩到 0.2500），因为高词法权重会把 BM25 噪声灌进来。

**正解是计划 1.3 里那条被搁置的建议："标识符识别时把词法权重临时拉高"** —— 但必须是**定向**的，
不是全局的。实现为**精确符号命中加成**（`SymbolMatchBoost`）：

| 配置 | R@5 | R@10 | 宽松R@10 | MRR | Hits@1 | 两腿都没捞到 |
|---|---|---|---|---|---|---|
| e5 融合 0.3/0.7 | 0.4750 | 0.5000 | 0.5625 | 0.4042 | 14/40 | 12 |
| qwen3 0.3/0.7 sk5 boost0 | 0.4750 | 0.4750 | 0.6000 | 0.3263 | 9/40 | 5 |
| qwen3 sk100 boost0 | 0.4750 | 0.4750 | 0.6000 | 0.3263 | 9/40 | 3 |
| qwen3 sk100 boost0.3 | 0.5375 | 0.5500 | 0.6250 | 0.3871 | 11/40 | 3 |
| **qwen3 sk100 boost0.6（已发布）** | **0.5375** | **0.5750** | **0.6500** | **0.4573** | **15/40** | **3** |
| qwen3 sk100 boost1.0 | 0.5375 | 0.5750 | 0.6500 | 0.4573 | 15/40 | 3 |

0.6 与 1.0 完全同值 ⇒ 加成已饱和，取更保守的 0.6。

### 21.3 最终对比（`tests/baseline-qwen3.json`，对比 `tests/m1-fused-default.json`）

| 指标 | e5 融合 | **Qwen3 + 新默认** | 变化 |
|---|---|---|---|
| Recall@5 | 0.4750 | **0.5750** | **+10.0 pt** |
| Recall@10 | 0.5000 | **0.5750** | **+7.5 pt** |
| 宽松 Recall@10 | 0.5625 | **0.6500** | +8.75 pt |
| MRR | 0.4042 | **0.4573** | **+13.1%** |
| nDCG@10 | 0.4280 | **0.4779** | **+11.7%** |
| Context Precision@10 | 0.4042 | **0.4462** | **+10.4%** |
| Hits@1 | 14/40 | **15/40** | +1 |
| **候选生成缺口（两腿都没捞到）** | **12** | **3** | **−75%** |
| 延迟 | p50 59 ms | p50 79 ms | +34%（可接受） |

**每一项指标都提升。**

### 21.4 分类拆解（这才是真相）与 holdout 证据

| 类别 | e5 R@10 | **Qwen3 R@10** | e5 MRR | **Qwen3 MRR** |
|---|---|---|---|---|
| exact-symbol | 1.000 | 0.875 | 0.667 | **0.883** |
| def-name | 1.000 | 0.875 | **0.938** | 0.723 |
| natural-language | 0.125 | **0.125** | 0.125 | 0.085 |
| cross-layer | 0.250 | **0.625** | 0.250 | 0.221 |
| **bilingual-mod** | 0.125 | **0.375（3×）** | 0.042 | **0.375（9×）** |
| **全部 40 条** | 0.500 | **0.575** | 0.404 | **0.457** |
| **holdout 10 条** | 0.500 | **0.700** | — | — |

三条结论：
1. **计划对 Qwen3 收益点的判断被证实**：`bilingual-mod` 0.125 → 0.375（MRR 提升 9 倍）、
   `cross-layer` 0.25 → 0.625 —— 中文/多语言与"关系型提问"正是它的强项。
2. **holdout 提升幅度（+20pt）大于调参集（+7.5pt）** ⇒ **不是过拟合**，是真的泛化改善。
3. **`natural-language` 仍停在 0.125** —— 这是**唯一没被解决的短板**，且它不随模型换代改善。
   下一步该在这里投入（查询改写 / 让 agent 先 `grep` 拿标识符再 `rough_search`），而不是继续换模型。

### 21.5 交付的默认值（都已写进代码常量）

```csharp
DefaultSemanticCandidates = 100   // 5 -> 100：Qwen3 下宽松召回 0.5625 -> 0.6000，缺口 5 -> 3
DefaultSymbolMatchBoost   = 0.6   // 精确符号命中加成（仅当查询形如标识符时启用）
DefaultLexicalWeight      = 0.3   // 两个模型下都是甜点；0.5/0.5 劣于纯语义
DefaultSemanticWeight     = 0.7
UseSemanticScoringOnly    = false // 融合，不是纯语义
RerankCandidates          = 0     // 重排默认关闭（§20 实测否决）
```

### 21.6 三个"评测脚手架覆盖生产默认值"的坑（同一类问题踩了三次）

| # | 位置 | 症状 |
|---|---|---|
| 1 | `bench --symbol-boost` 默认 0.0 | 显式覆盖了配置的 0.6，最终存档跑的还是无提权配置 |
| 2 | `bench-retrieval.ps1 -SemanticK` 默认 5 | 覆盖了配置的 100 |
| 3 | `bench` 未传 `--hybrid` 时强制纯语义 | `smoke-gold.ps1` 因此测的是纯语义路径，金标 2/5 未命中 |

统一修法：**脚手架只在调用方显式给了参数时才传，否则一律继承检索器的配置默认值**；
`bench` 现在默认就是生产配置（新增 `--semantic-only` 显式选旧模式）。

> 教训：评测脚本的默认值必须**等于生产默认值**，否则"测过的"和"发布的"是两套东西。

### 21.7 顺带修掉的真问题：bf16 归一化不精确

验收第 3 步发现 Qwen3 的向量 `|v| = 1.0012`（不是 1.0），且重复调用不一致。
原因：`normalize_embeddings=True` 在 **bfloat16** 下归一化，模长只近似为 1，会让每个点积偏 ~0.1%。
修法：服务端改为**在 float32 里归一化**（`_normalize_fp32`）→ 模长精确 1.0；
测试容差同时调整到 bf16 的合理水平（重复调用用余弦 1e-3，而非 1e-6）。

### 21.8 运行手册已执行完毕

端口 5000 已从 e5 换成 Qwen3（**用 venv 的 python**，见 §17.2 的陷阱）；
MCP server 已重启并拾取 Qwen3 索引 + 本节所有默认值 + `grep` 工具 + L3 遥测。
**线上验证**：`rough_search("CompPowerTrader")` → rank 1 正是 `RimWorld.CompPowerTrader`（冷启 1.15 s，随后 0.18 s）。
